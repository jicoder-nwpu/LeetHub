...
public interface SolutionMapper {
    ...
    @Select("select s.*, p.title as ptitle, p.difficulty from solution s, problem p where user_id=#{user_id} and s.problem_id = p.problem_id and s.solution_id != #{solution_id} order by update_time desc limit #{count}")
    @Results(value = {
            @Result(column = "ptitle", property = "problem.title"),
            @Result(column = "difficulty", property = "problem.difficulty")
    })
